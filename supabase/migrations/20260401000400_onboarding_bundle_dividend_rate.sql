-- Add p_dividend_rate parameter to driver_create_onboarding_bundle.
-- Allows drivers to specify the merchant's dividend rate at machine onboarding time.

-- Revoke the old function signature first.
...
DROP FUNCTION IF EXISTS public.driver_create_onboarding_bundle(TEXT, TEXT, TEXT, UUID, TEXT, TEXT, TEXT, INTEGER, NUMERIC, TEXT[], TEXT);
...
BEGIN
  ...
  -- Use provided dividend rate or fall back to the column default (0.30).
  v_dividend_rate := COALESCE(p_dividend_rate, 0.30);

  IF v_dividend_rate < 0 OR v_dividend_rate > 1 THEN
    RAISE EXCEPTION 'dividend rate must be between 0 and 1';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.driver_create_onboarding_bundle(TEXT, TEXT, TEXT, UUID, TEXT, TEXT, TEXT, INTEGER, NUMERIC, TEXT[], TEXT, NUMERIC) TO authenticated;
